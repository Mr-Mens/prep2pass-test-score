#!/usr/bin/env node
/**
 * Delete a Supabase Auth user (and cascaded profile rows) by email.
...
if (!emailArg) {
  console.error("Usage: node scripts/delete-auth-user.mjs user@example.com");
  process.exit(1);
}
...
async function main() {
  ...
  if (!user) {
    console.log(`No auth user found for ${emailArg}`);
    return;
  }
